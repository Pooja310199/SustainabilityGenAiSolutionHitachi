    ...
    >
      ...
      {expandedSections[categoryKey] && (
        <div style={{ padding: "12px 18px" }}>
          ...
          {isGeo &&
            (content.items || [])
              ...
              .map((item, i) => (
                <div key={i}>
                  <SeverityDot level={item.severity} />{" "}
                  <strong>{item.description}</strong>: {item.summary}
                </div>
              ))}
            ...
            <div style={{ padding: "12px 18px" }}>
              {(() => {
                ...
                return (
                  <>
                    <div
                      onClick={() => toggle(setExpandedMetrics, metricsKey)}
                      ...
                      <div style={{ marginLeft: 10, marginTop: 8 }}>
                        {Object.entries(content.metrics)
                          ...
                          .map(([metric, val]) => {
                            ...
                            if (
                              ...
                              return (
                                <div key={metric} style={{ marginBottom: 12 }}>
                                  <div
                                    onClick={() =>
                                      toggle(setExpandedSubIndicators, subKey)
                                    }
                                    style={{
                                      cursor: "pointer",
                                      fontWeight: 600,
                                    ...
                                    >
                                      {Object.entries(val)
                                        ...
                                        .map(([indexName, indexVal], i) => (
                                          ...
                                          >
                                            <SeverityDot
                                              level={indexVal.severity}
                                            />{" "}
                                            <strong>
                                              {capitalizeWords(
                                                indexName.replaceAll("_", " "),
                                              )}
                                            </strong>{" "}
                                            →{" "}
                                            {indexVal.value && (
                            ...
                            if (
                              ...
                              return (
                                <div key={metric} style={{ marginBottom: 12 }}>
                                  <div
                                    onClick={() =>
                                      toggle(setExpandedSubIndicators, fhKey)
                                    }
                                    style={{
                                      cursor: "pointer",
                                      fontWeight: 600,
                                    ...
                                    >
                                      {Object.entries(val)
                                        ...
                                        .map(([subMetric, subVal], i) => (
                                          ...
                                          >
                                            <SeverityDot
                                              level={subVal.severity}
                                            />{" "}
                                            <strong>
                                              {capitalizeWords(subMetric)}
                                            </strong>{" "}
                                            →{" "}
                                            {subVal.score &&
                                              subVal.score !== "N/A" && (
                            ...
                            if (
                              metric ===
                                "ibrahim_index_of_african_governance" &&
                              typeof val === "object"
                            ) {
                            ...
                            const overallSeverity =
                              val.severity ||
                              (typeof val === "object"
                                ? Object.values(val).find((v) => v?.severity)
                                    ?.severity
                                : "N/A");
                                ...
                                <div style={{ flex: 1 }}>
                                  ...
                                  {Object.entries(val)
                                    .filter(
                                      ([k, v]) =>
                                        typeof v !== "object" &&
                                        !k.toLowerCase().includes("rescaled") &&
                                        v !== "N/A" &&
                                        v !== "No Data" &&
                                        v !== "" &&
                                        v !== null,
                                    ...
                                    .map(([k, v], i) => (
                                      <span key={i} style={{ marginLeft: 12 }}>
                                        <strong>{capitalizeWords(k)}:</strong>{" "}
                                        {v}
                                      </span>
                                    ))}
                                </div>
          ...
          {/* Trends & Challenges */}
          {((content.analysis && content.analysis.trends_and_challenges) ||
            content.trends_and_challenges) && (
            <div style={{ marginTop: 12, marginLeft: 18 }}>
              <strong>Trends & Challenges:</strong>
              <ul style={{ marginTop: 5, marginLeft: 18 }}>
                {content.analysis?.trends_and_challenges?.map((t, i) => (
                  <li key={`analysis-${i}`}>{t}</li>
                ))}
                {content.trends_and_challenges?.map((t, i) => (
                  <li key={`top-${i}`}>{t}</li>
                ))}
              </ul>
            </div>
          ...
          {isRuleOfLaw &&
            content.analysis?.legal_and_institutional_framework && (
              <div style={{ marginTop: 10, marginLeft: 18 }}>
                {" "}
                <strong>Legal & Institutional Framework:</strong>{" "}
                <ul style={{ marginTop: 5, marginLeft: 18 }}>
                  {" "}
                  {content.analysis.legal_and_institutional_framework.map(
                    (t, i) => (
                      <li key={i}>{t}</li>
                    ),
                  )}{" "}
          ...
          {Array.isArray(content.sources) &&
            ...
            (() => {
              ...
              return (
                <>
                  <div
                    onClick={() => toggle(setExpandedSources, sourceKey)}
                    style={{
                      cursor: "pointer",
                      fontWeight: 500,
                      marginTop: 12,
