  ...
  return (
    ...
    >
      <div className="font-semibold text-gray-900">
        {item.title || item.case_title || item.name || "Untitled"}
      </div>
      ...
      {item.type_of_allegation && (
        <div>
          <span className="font-medium">Type:</span> {item.type_of_allegation}
        </div>
      )}

      {item.source_type && (
        <div>
          <span className="font-medium">Source Type:</span> {item.source_type}
        </div>
      )}
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="ml-3 mt-1 text-sm space-y-1">
                              {metric.query && (
                                <div>
                                  <b>Query:</b> {metric.query}
                                </div>
                              )}
                              {metric.result && (
                                <div>
                                  <b>Result:</b> {metric.result}
                                </div>
                              )}
                              {metric.reasoning && (
                                <div>
                                  <b>Reasoning:</b> {metric.reasoning}
                                </div>
                              )}
                              {metric.risk_level && (
                                <div>
                                  <b>Risk:</b> {metric.risk_level}
                                </div>
                              )}
                            </div>
                          </div>
                    ...
                    {result.analysis &&
                      Object.entries(result.analysis).map(
                        ([groupKey, groupData]) => {
                          ...
                          const isAmnesty =
                            result.sub_category ===
                            "Amnesty Corporate Crimes Handbook";

                          const isAllowedEmptyGroup =
                            isBHRRC || (isAmnesty && groupKey === "cases");

                          const items = Array.isArray(groupData.items)
                            ? groupData.items
                            : [];

                          // ❗ IMPORTANT: do NOT return null if items are empty
                          // Only skip completely unrelated empty groups
                          if (!items.length && !isAllowedEmptyGroup)
                            return null;
